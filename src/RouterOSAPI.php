...
//new source by.Acil
function convertLimitUptime($limit_uptime)
{
    $time_units = [
        's' => ' Detik',
        'm' => ' Menit',
    ...
    preg_match_all('/(\d+)([whdms])/i', $limit_uptime, $matches, PREG_SET_ORDER);

    foreach ($matches as $match) {
        if ($match[2] === 'w') {
            $total_days += intval($match[1]) * 7;
        } elseif ($match[2] === 'd') {
            $total_days += intval($match[1]);
        } elseif ($match[2] === 'h') {
            $total_hours += intval($match[1]);
        } elseif ($match[2] === 'm') {
            $total_minutes += intval($match[1]);
        } elseif ($match[2] === 's') {
            $total_seconds += intval($match[1]);
        }
    }
    ...
    $months = floor($total_days / 30); // 30 days considered as a month
    $remaining_days = $total_days % 30;

    $years = floor($total_days / 365); // 365 days considered as a year
    ...
    if ($remaining_days > 0) {
        $result .= $remaining_days . ' Hari';
    }

    if ($total_hours > 0) {
        if ($years > 0 || $months > 0 || $remaining_days > 0) {
            $result .= ', ';
        }
        $result .= $total_hours . ' Jam';
    }

    if ($total_minutes > 0) {
        if ($years > 0 || $months > 0 || $remaining_days > 0 || $total_hours > 0) {
            $result .= ', ';
        }
        $result .= $total_minutes . ' Menit';
    }

    if ($total_seconds > 0) {
        if ($years > 0 || $months > 0 || $remaining_days > 0 || $total_hours > 0 || $total_minutes > 0) {
            $result .= ', ';
        }
        $result .= $total_seconds . ' Detik';
    }
...
//new source by.Acil
function convertToFullMonthName($monthAbbreviation)
{
    $monthAbbreviation = ucfirst(strtolower($monthAbbreviation));
    $monthMapping = [
        'Jan' => 'Januari',
        'Feb' => 'Februari',
        'Mar' => 'Maret',
    ...
    return isset($monthMapping[$monthAbbreviation]) ? $monthMapping[$monthAbbreviation] : '';
}
//new source by.Acil
